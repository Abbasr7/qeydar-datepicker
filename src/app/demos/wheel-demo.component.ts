    ...
    <div class="slot-demo wheel-demo">
      ...
      <div class="wheel-preview" [dir]="rtl ? 'rtl' : 'ltr'">
          ...
          <ng-template qeydarTemplate="body" let-context>
            ...
            <div class="wheel {{ rootClass }}" role="group" aria-label="Wheel date picker">
              ...
              <div class="wheel-col wheel-col--month" (scroll)="onColumnScroll($event, 'month')">
                ...
                <button
                  type="button"
                  *ngFor="let m of monthList; let i = index; trackBy: trackValue"
                  class="wheel-item"
              ...
              <div class="wheel-col wheel-col--year" (scroll)="onColumnScroll($event, 'year')">
                ...
                <button
                  type="button"
                  *ngFor="let y of yearList; trackBy: trackValue"
                  class="wheel-item"
          ...
          <ng-template qeydarTemplate="footer" let-context>
            <div class="wheel-footer">
              <span class="wheel-footer-hint">{{
                calendarType === 'jalali' ? 'تاریخ را تایید کنید' : 'Confirm the date'
              }}</span>
              <div class="wheel-footer-actions">
                ...
                >{{ calendarType === 'jalali' ? 'انصراف' : 'Cancel' }}</button>
                <button
                  type="button"
                  class="wheel-btn wheel-btn--primary"
                  (click)="onConfirm(context)"
                >{{ calendarType === 'jalali' ? 'تایید' : 'Done' }}</button>
              </div>
            </div>
          </ng-template>
        </qeydar-date-picker>
        ...
        <output class="wheel-output" aria-live="polite">
          <span>{{ calendarType === 'jalali' ? 'مقدار انتخاب‌شده' : 'Selected value' }}</span>
...
export class WheelDemoComponent implements AfterViewChecked, OnDestroy {
  ...
  isToday(part: 'day' | 'month' | 'year', value: number): boolean {
    if (this.todayYear === null || this.todayMonth === null || this.todayDay === null) {
      return false;
    }
    if (part === 'day') {
      return value === this.todayDay && this.activeMonth === this.todayMonth && this.activeYear === this.todayYear;
    }
    if (part === 'month') {
  ...
  formatOutput(): string {
    ...
    return this.adapter.format(date, this.calendarType === 'jalali' ? 'yyyy/MM/dd' : 'MM/dd/yyyy');
  }
